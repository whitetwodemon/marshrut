...
class DetailsController
{
    ...
    public static function index(array $params): void
    {
        ...
        foreach ($details as &$detail) {
            $ops = $db->prepare(
                'SELECT num, name, work_center, time_min, setup_time_min, comment, requires_cnc
                   FROM operations
                  WHERE detail_id = :id
                  ORDER BY num'
            );
    ...
    public static function create(array $params): void
    {
        ...
        try {
            ...
            if (!empty($body['operations'])) {
                $ins = $db->prepare(
                    'INSERT INTO operations (detail_id, num, name, work_center, time_min, setup_time_min, comment, requires_cnc)
                     VALUES (:did, :num, :name, :wc, :time, :setup, :comment, :cnc)'
                );
                foreach ($body['operations'] as $op) {
                    $ins->execute([
                        ...
                        ':setup' => (int) ($op['setup_time_min'] ?? $op['setup'] ?? 0),
                        ':comment' => mb_substr((string)($op['comment'] ?? ''), 0, 500) ?: null,
                        ':cnc' => !empty($op['requires_cnc']) ? 1 : 0,
                    ]);
                }
            }
            ...
            if (isset($body['operations'])) {
                ...
                foreach ($body['operations'] as $op) {
                    $ins->execute([
                        ...
                        ':setup' => max(0, (int) ($op['setup_time_min'] ?? $op['setup'] ?? 0)),
                        ':comment' => mb_substr(sanitize_string($op['comment'] ?? '', 500), 0, 500) ?: null,
                        ':cnc' => !empty($op['requires_cnc']) ? 1 : 0,
                    ]);
                }
            }
