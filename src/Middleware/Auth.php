...
class Auth
{
    ...
    {
        $s = getenv('JWT_SECRET');
        if (!$s || $s === 'CHANGE_ME' || $s === 'marshrut-secret-change-in-production') {
            // Небезопасная конфигурация: секрет не задан или дефолтный → токены подделываемы.
            error_log('[SECURITY] JWT_SECRET не задан или дефолтный — задайте сильный секрет в .env!');
            return 'marshrut-secret-change-in-production';
        }
        return $s;
    }
    ...
    /** Restrict operators to work-center routes only */
    public static function requireNotOperator(): object
    {
        $user = self::require();
        if ($user->role === 'operator') {
            \Marshrut\json_out(['error' => 'Доступ запрещён'], 403);
        }
        return $user;
    }
