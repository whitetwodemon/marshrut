...
function Scanner({ data, tasks, scanLog, lang, qrSize, onScanResult, users }) {
  ...
  async function confirmClose(qty, operator, action, comment, closeStatus) {
    ...
    setClosing(true);
    try {
      if (closeStatus === 'problem') {
        // Проблема: останавливаем ВЕСЬ заказ (статус → problem), а не только операцию
        const orderId = detected.task.orderId;
        if (!comment || !comment.trim()) { alert('Укажите причину проблемы в комментарии'); setClosing(false); return; }
...
function CloseOpModal({ task, detail, qrText, lang, closing, action, onConfirm, onCancel, users }) {
  const S = useStrings(lang);
  // По умолчанию закрываем операцию ПОЛНОСТЬЮ (planned шт).
  // qty = итоговое кол-во выполненных штук (не остаток) — согласуется с close() на бэке.
  const [qty, setQty]           = React.useState(Number(task.planned) > 0 ? Number(task.planned) : 1);
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, marginTop:4 }}>
              <div className="field">
                ...
                <select className="select" value={closeStatus} onChange={e=>setCloseStatus(e.target.value)} style={{width:'100%'}}>
                  ...
                  <option value="problem">{lang === 'en' ? '⚠ Problem (stop order)' : '⚠ Проблема — остановить заказ'}</option>
                </select>
              </div>
              <div className="field">
