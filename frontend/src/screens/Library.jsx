        ...
        <React.Fragment key={op.num + '_' + idx}>
          ...
          {idx === 0 && addAfter === -1 && (
            <InsertOpRow form={newForm} setForm={setNewForm}
              onConfirm={confirmInsert} onCancel={()=>setAddAfter(null)} saving={saving}/>
          )}
...
function InsertOpRow({ form, setForm, onConfirm, onCancel, saving }) {
  return (
    <div style={{ display:'flex', gap:6, padding:'8px 12px', background:'rgba(217,72,15,.06)',
      ...
      <select className="select" value={newForm.workCenter}
        onChange={e=>setNewForm(p=>({...p,workCenter:e.target.value}))} style={{ flex:1 }}>
        <option value="">Выбрать РЦ…</option>
        {(workCenters||[]).map(w=><option key={w.id} value={w.code}>{w.code} — {w.name}</option>)}
      </select>
      ...
      <button className="btn primary" onClick={onConfirm} disabled={saving} style={{fontSize:12}}>
        {saving ? '…' : '+ Добавить'}
      </button>
...
export function Library({ data, tasks, lang, onNewDetail, onEditDetail, onDeleteDetail, workCenters, onOpenOrder }) {
  const S = useStrings(lang);
  const [selectedId, setSelectedId] = React.useState(() => data.details[0]?.id || null);
  const [query, setQuery] = React.useState('');
  const [showFilters, setShowFilters] = React.useState(false);
  const [fMaterial, setFMaterial] = React.useState('');
  const [fInOrders, setFInOrders] = React.useState(false);
  const [fSort, setFSort] = React.useState('name');

  const allMaterials = React.useMemo(
    () => [...new Set(data.details.map(d => d.material).filter(Boolean))].sort(),
    [data.details]);
  const detailsInOrders = React.useMemo(
    () => new Set(tasks.map(t => t.detailId)), [tasks]);

  const filtered = data.details.filter(d =>
    ((d.name||'').toLowerCase().includes(query.toLowerCase()) ||
     (d.code||'').toLowerCase().includes(query.toLowerCase()) ||
     (d.material||'').toLowerCase().includes(query.toLowerCase()))
    && (!fMaterial || d.material === fMaterial)
    && (!fInOrders || detailsInOrders.has(d.id))
  ).sort((a, b) => fSort === 'code'
  ...
  const detailOrders = React.useMemo(() => {
    const ids = [...new Set(tasks.filter(t => t.detailId === detail.id).map(t => t.orderId))];
    return ids.map(oid => (data.orders || []).find(o => o.id === oid) || { id: oid, number: oid });
  }, [tasks, detail.id, data.orders]);
  const inOrders = detailOrders.length;
  ...
  return (
    <>
      <div className="page-head">
        ...
        <div className="row">
          <button className={'btn' + ((fMaterial || fInOrders || fSort !== 'name') ? ' primary' : '')}
            onClick={() => setShowFilters(f => !f)}>
            <Icon name="filter" size={14} />{lang === 'en' ? 'Filters' : 'Фильтры'}
          </button>
      ...
      {showFilters && (
        <div className="card" style={{ padding: 14, marginBottom: 14, display: 'flex', gap: 16, flexWrap: 'wrap', alignItems: 'center' }}>
          <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13 }}>
            Материал:
            <select className="input" value={fMaterial} onChange={e => setFMaterial(e.target.value)} style={{ minWidth: 160 }}>
              <option value="">Все</option>
              {allMaterials.map(m => <option key={m} value={m}>{m}</option>)}
            </select>
          </label>
          <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, cursor: 'pointer' }}>
            <input type="checkbox" checked={fInOrders} onChange={e => setFInOrders(e.target.checked)} />
            Только в активных заказах
          </label>
          <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13 }}>
            Сортировка:
            <select className="input" value={fSort} onChange={e => setFSort(e.target.value)}>
              <option value="name">По названию</option>
              <option value="code">По коду</option>
            </select>
          </label>
          {(fMaterial || fInOrders || fSort !== 'name') && (
            <button className="btn" style={{ marginLeft: 'auto' }}
              onClick={() => { setFMaterial(''); setFInOrders(false); setFSort('name'); }}>Сбросить</button>
          )}
        </div>
      )}
        ...
        <div className="lib-detail">
          <div className="lib-detail-head">
            ...
            <div className="row" style={{ marginTop: 8, gap: 8 }}>
              ...
              {detailOrders.map(o => (
                <button key={o.id} className="pill prog" title={lang === 'en' ? 'Open order' : 'Открыть заказ'}
                  onClick={() => onOpenOrder && onOpenOrder(o.id)}
                  style={{ cursor: 'pointer', border: 'none', font: 'inherit' }}>
                  <span className="dot" />{o.number || o.id} →
                </button>
              ))}
