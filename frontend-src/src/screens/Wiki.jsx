...
function WikiPage() {
  ...
  const CONTENT = {
    ...
    // ── О разработчике ───────────────────────────────────────────────────
    admin: makeContent('Администрирование', [
      ['Пользователи', 'Вкладка «Пользователи»: создание, редактирование, блокировка. При создании по умолчанию назначается роль «Оператор». Пользователь с ролью admin удалить нельзя.'],
      ['Роли и права', 'Вкладка «Роли»: просмотр списка прав каждой роли. admin — всё. foreman — всё кроме управления пользователями. operator — работа с заданиями и сканер. viewer — только просмотр.'],
      ['Рабочие центры', 'Вкладка «Рабочие центры»: добавление новых РЦ, изменение кода и названия. Новые РЦ сразу появляются в выпадающих списках при создании операций.'],
      ['Настройки системы', 'Вкладка «Настройки»: часовой пояс (для корректного отображения времени смен), название предприятия, пороги нормоконтроля (%), расписание смен, максимум попыток входа.'],
      ['Часовой пояс', 'Выберите часовой пояс вашего региона в Настройках → Время и регион. По умолчанию UTC+3 (Москва). Влияет на отображение времени начала смен, операций, журнала.'],
    ]),
    about: (
      <div>
